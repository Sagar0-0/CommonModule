...
//    val bits: Int = (ALL_DAYS and this.encodedBits)
    ...
    private fun toString(context: Context, order: Order, forceLongNames: Boolean): String {
        if (!isRepeating) {
            return context.getString(StringR.string.repeat_one)
        }

        if ((ALL_DAYS and this.encodedBits) == ALL_DAYS) {
            return context.getString(StringR.string.every_day)
        }
        ...
        val separator: String = context.getString(StringR.string.day_concat)
