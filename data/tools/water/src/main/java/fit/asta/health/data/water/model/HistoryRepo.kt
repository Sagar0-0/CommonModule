...
class HistoryRepo @Inject constructor(val historyDao: HistoryDao) {
    ...
    fun getAllConsumptionHistory(date : String) : Flow<List<ConsumptionHistory>> = historyDao.getAllConsumptionHistory(date)
    ...
    suspend fun insertConsumptionData(bevData : ConsumptionHistory) = historyDao.insertConsumedData(bevData)
}