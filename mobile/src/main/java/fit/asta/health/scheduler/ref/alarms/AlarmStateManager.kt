...
import fit.asta.health.scheduler.ref.AsyncHandler
...
class AlarmStateManager : BroadcastReceiver() {
    ...
    private class AlarmManagerStateChangeScheduler : StateChangeScheduler {
        ...
        override fun scheduleInstanceStateChange(
            ...
            val stateChangeIntent: Intent =
                asm.createStateChangeIntent(context, "ALARM_MANAGER", instance, newState)
        ...
        override fun cancelScheduledInstanceStateChange(context: Context, instance: AlarmInstance) {
            ...
            val pendingIntent: PendingIntent? =
                PendingIntent.getService(
                    context, instance.hashCode(),
                    asm.createStateChangeIntent(context, "ALARM_MANAGER", instance, null),
    ...
    fun createStateChangeIntent(
        ...
        intent.action = "change_state" //CHANGE_STATE_ACTION
        intent.addCategory(tag)
        intent.putExtra(
            "intent.extra.alarm.global.id",
            -1
        )// DataModel.dataModel.globalIntentId) ALARM_GLOBAL_ID_EXTRA
        if (state != null) {
            intent.putExtra("intent.extra.alarm.state", state.toInt())//ALARM_STATE_EXTRA
        }
        return intent
    }
    ...
    private val ALARM_GLOBAL_ID_EXTRA = "intent.extra.alarm.global.id"
    ...
    private val ALARM_MANAGER_TAG = "ALARM_MANAGER"
        ...
        scope.launch {
            alarmRefDao.getAlarmById(instance.mAlarmId!!)?.let { alarm ->
                if (!alarm.daysOfWeek.isRepeating) {
                    ...
                    var nextRepeatedInstance = createInstanceAfter(currentTime, alarm)
                    if (instance.mAlarmState > InstancesColumns.FIRED_STATE &&
                        nextRepeatedInstance.alarmTime == instance.alarmTime
                    ) {
                        nextRepeatedInstance = createInstanceAfter(instance.alarmTime, alarm)
                    }
...
//            val instance: AlarmInstance? = AlarmInstance.getInstance(AlarmInstance.getId(uri))
            scope.launch {
                alarmInstanceDao.getInstance(1)?.let { instance -> //// get from intent
                    val globalId = -1
            ...
            val uri: Uri = intent.data!!
            scope.launch {
                val instance: AlarmInstance? = alarmInstanceDao.getInstance(1)
                if (instance == null) {
...
//                    Open DeskClock which is now positioned on the alarms tab.
    ...
    fun createInstanceAfter(time: Calendar, alarm: Alarm): AlarmInstance {
        val nextInstanceTime = alarm.getNextAlarmTime(time)
